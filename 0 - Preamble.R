...
DataDirs <- c(Dir.Data, Dir.Ranges, Dir.Shapes, Dir.PFTC, Dir.Ranges.Amphibians, Dir.Ranges.Birds, Dir.Ranges.Mammals, Dir.Ranges.Plants, Dir.Ranges.Reptiles)
CreateDir <- sapply(DataDirs, function(x) if(!dir.exists(x)) dir.create(x))
## EXPORTS
Dir.Exports <- file.path(Dir.Base, "2 - Exports")
Dir.Plots <- file.path(Dir.Exports, "Plots")
...
ExportDirs <- c(Dir.Exports, Dir.Plots, Dir.Plots.FIA, Dir.Richness, Dir.Richness.Current)
